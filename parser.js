...
class Parser {
 ...
 eat(type) {
  ...
  statement() {
    if (this.current.type === 'TASK' && this.peekType() === 'FUNC') {
    this.eat('TASK');
    this.eat('FUNC');
    return this.asyncFuncDeclaration();
  }
    switch (this.current.type) {
      case 'DEFINE': return this.varDeclaration();
      case 'SHOW': return this.sldeployStatement();
      ...
      case 'DEPLOY': return this.deployStatement();
...
deployStatement() {
  ...
  this.eat('DEPLOY');
  if (this.current.type === 'FUNC' || this.current.type === 'DEFINE') {
    const declaration = this.statement();
    return {
      type: 'DeployStatement',
  ...
  if (this.current.type === 'LBRACE') {
    ...
    while (this.current.type !== 'RBRACE') {
      if (this.current.type !== 'IDENTIFIER') {
        throw new ParseError("Expected identifier in deploy list", this.current, this.source);
      }
...
varDeclaration() {
  ...
  if (this.current.type !== 'IDENTIFIER') {
    throw new ParseError(
      ...
      this.current,
      this.source,
      "Variable declarations must be followed by an identifier, e.g. define x = 5"
    );
  }
...
sldeployStatement() {
  ...
  if (this.current.type !== 'LPAREN') {
    throw new ParseError(
      ...
      this.current,
      this.source,
      "Show statements require parentheses, e.g. show(...);"
    );
  }
  this.eat('LPAREN');
  ...
  if (this.current.type === 'RPAREN') {
    throw new ParseError(
      "Expected expression inside show parentheses",
      this.current,
      this.source,
      "Show statements require a value or expression, e.g. show(message);"
    );
  }

  const expr = this.expression();
  if (expr.type === 'Literal' && typeof expr.value === 'string' && expr.value === "") {
    throw new ParseError(
      "Empty string literals are not allowed inside show parentheses",
      t,
      this.source,
      "Provide a non-empty string or a valid expression."
    );
  }

  if (this.current.type !== 'RPAREN') {
    throw new ParseError(
      "Expected ')' after show expression",
      this.current,
      this.source,
      "Close the show expression with ')'"
    );
  }
  this.eat('RPAREN');
  ...
  if (this.current.type !== 'SEMICOLON') {
    throw new ParseError(
      "Missing semicolon after 'show' statement",
      this.current,
      this.source,
...
forStatement() {
  ...
  if (
    (this.current.type === 'DEFINE' && this.peekType() === 'IDENTIFIER' && this.peekType(2) === 'IN') ||
    (this.current.type === 'IDENTIFIER' && this.peekType() === 'IN')
  ) {
    let variable;
    let variableLine, variableColumn;
    let iterable;
    let defineKeyword = false;

    if (this.current.type === 'DEFINE') {
      letKeyword = true;
      this.eat('DEFINE');
    }
...
if (this.current.type === 'urlencode' || this.current.type === 'URLENCODE') {
    return this.urlencodeExpression();
}
if (t.type === 'SELF') {
  this.eat('SELF');
